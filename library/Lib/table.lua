...
table.merge = function(table1, table2)
    ...
    if (table2 == nil) then return tempTable end
    for _, v in ipairs(table2) do table.insert(tempTable, v) end
    return tempTable
end
...
table.includes = function(val, arr)
    local isin = false
    if (val == nil or #arr <= 0) then return isin end
...
table.delete = function(val, arr, qty)
    ...
    for k, v in ipairs(arr) do
        if (v == val) then
            ...
            table.remove(arr, k)
            k = k - 1
            if (qty ~= -1 and q >= qty) then break end
        end
    end
end
