...
require "blizzard.System.UI.StartInterface"
require "blizzard.System.UI.ChoiceHeroInterface"
require "library.API"
...
CJ.TriggerAddCondition(trig_Control, CJ.Condition(function()
...
CJ.TriggerAddAction(trig_MainControl, function()
    ...
    if com:GetData("界面状态") == "初始界面" then
        -- statements
        StartInterface.Chick(u,zdy)
    end
    if com:GetData("界面状态") == "选择职业" then
        -- statements
        ChoiceHeroInterface.Chick(u,zdy)
    end
    -- if (zdy == 1) then
    --     if com:GetData("游戏状态") == "初始化" then
    --         -- statements
